# MNE-Python — capability map & how EEGvis surfaces it
...
Companion docs: [`BUILD_PLAN_V2.md`](BUILD_PLAN_V2.md) (compounding systems +
phased todos), [`wireframes/`](wireframes/) (the v2 UI deck).
...
transitions; the UI's left rail is literally this graph.
...
**Old model (v1):** one fixed grid, one toolbar of preprocessing dropdowns, panels
hard-wired. Adding anything means editing the shell.

**New model:** the left rail is the container graph (§1.1). Selecting a container
sets the **active object**. The workspace shows a **canvas of view-cards** valid
for that object; the **command bar** offers only operations whose input type
matches it; the **right inspector** shows the selected sub-thing (channel /
component / label / step). One transport + provenance strip spans the bottom.

```
┌─ command bar (⌘K + visible "+ Operation")────────────────────────────────────┐
├────────┬───────────────────────────────────────────────────────┬─────────────┤
│        │                                                       │             │
│ CONT-  │           CARD CANVAS                                  │  INSPECTOR  │
│ AINER  │   modular, add/remove/arrange view-cards               │  context-   │
│ RAIL   │   each card = one plot from §1.4                       │  sensitive  │
│        │   subscribes to shared cursor + active container       │             │
│ Raw ●  │                                                       │  channel /  │
│  Epochs│   ┌────────────┐ ┌────────────┐ ┌──────────┐          │  component /│
│  Evoked│   │ waveform   │ │ sensors    │ │ PSD      │          │  label /    │
│  Spec  │   └────────────┘ └────────────┘ └──────────┘          │  step       │
│  TFR   │                                                       │             │
│  ICA   │                                                       │             │
│  Source│                                                       │             │
├────────┴───────────────────────────────────────────────────────┴─────────────┤
│ TRANSPORT  ◀◀ �I▶ ▶▶   0:05 / 3:00   1× 2× 4×   [minimap ▓▓▒░░░]  ⟲ loop      │
│ PROVENANCE  ● ingest → ● 1–40 Hz → ● notch 60 → ● ICA −2 → ○ epoch  [py] [⤓] │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
- **Server-rendered image cards** (matplotlib-Agg or PyVista-offscreen → PNG,
  ...
  today's ICA topo PNGs — see [`BUILD_PLAN_V2.md`](BUILD_PLAN_V2.md) P0.5.
...
It's a real multi-phase feature ([`BUILD_PLAN_V2.md`](BUILD_PLAN_V2.md) **P6**),
...
| `set_channel_types`, `rename_channels`, `reorder_channels` | ⬜ | Channels table (inspector) | P2 |
| `pick` / `drop_channels` / `add_reference_channels` | ⬜ | Channels table | P2 |
| **`interpolate_bads`** (spherical spline) | ⬜ | Raw op — **closes the mark-bad loop** | **P2 first** |
| `compute_current_source_density` (Laplacian) | ⬜ | Filter & repair op | P5 |
| `find_bad_channels_lof` / RANSAC-style auto-bad | ⬜ | Artifact ID op | P3 |
...
| Capability | Status | UI home | Priority |
|---|---|---|---|
| `annotate_muscle_zscore`, `annotate_amplitude`, `annotate_break` | ⬜ | Artifact ID op → writes Annotations | P3 |
| `find_eog_events` / `find_ecg_events`, `create_eog/ecg_epochs` | ⬜ | Artifact ID op | P3 |
| `regress_artifact` (EOG regression) | ⬜ | Artifact ID op | P3 |
| Manual annotation create/edit/label on the waveform | ⬜ | Waveform card (annotation mode) | P2 |
...
| **ICLabel** auto-classification (`mne-icalabel`) | ⬜ | ICA workspace — badges + auto-exclude | **P3 first** |
...
| Capability | Status | UI home | Priority |
|---|---|---|---|
| `events_from_annotations`, `make_fixed_length_events` → Epochs | 🟡 (endpoint only, no UI) | Segment op + Events table | P4 |
| `find_events` (stim channel), `merge_events`, `define_target_events` | ⬜ | Segment op | P4 |
| `Epochs` reject/flat, `reject_by_annotation`, decim, detrend | ⬜ | Segment op params | P4 |
| autoreject (global + local) | ⬜ | Segment op | P4 |
| `make_metadata` + pandas-query epoch selection | ⬜ | Epochs workspace (metadata table) | P4 |
| `plot_image` / `plot_drop_log` / `plot_topo_image` | ⬜ | Epochs cards | P4 |
| `epochs.average()` (mean/median/robust), `combine_evoked`, `grand_average` | ⬜ | Average op | P4 |
| Evoked `plot_joint` / `plot_topomap` seq / `animate_topomap` / `plot_white` | ⬜ | Evoked cards (server PNG) | P4 |
| `plot_compare_evokeds` (conditions ± CI) | ⬜ | Evoked compare card | P4 |
| `get_peak`, GFP | ⬜ | Evoked inspector | P4 |
...
| **`tfr_morlet` / `tfr_multitaper` / `tfr_stockwell`** + ITC | ⬜ | TFR workspace | **P5** |
...
| `eegvis.launch(raw)` / `eegvis <file>` plugin + CLI | ⬜ | (packaging) | P8 |
